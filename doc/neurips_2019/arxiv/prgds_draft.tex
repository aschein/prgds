...
We study the inductive bias of the PRGDS by comparing its smoothing and forecasting abilities to those of the PGDS and two other baselines on a range of real-world count data sets of text, international events, and neural spike data. We find that the PRGDS often obtains better smoothing and forecasting performance than the PGDS and related baselines. Using a specific hyperparameter setting, the PRGDS permits the gamma states to take values of exactly zero, thereby encoding a unique inductive bias tailored to sparsity and burstiness. We find that this sparse variant always obtains better smoothing and forecasting performance than the non-sparse variant. We also find that this sparse variant yields a qualitatively broader range of latent structures---specifically, bursty latent structures that are highly localized in time.~\looseness=-1

% In \cref{sec:PRGDS} we introduce the PRGDS and discuss its properties. In \cref{sec:bg}, we detail connections to previous work. In \cref{sec:mcmc}, we derive inference. In \cref{sec:empirical}, we do experiments.
...
\textbf{Generative process.} The PRGDS is a form of canonical polyadic decomposition \cite{harshman1970foundations} that assumes~\looseness=-1
\begin{align}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/annotated_rg1.pdf}
...
{\label{fig:bessel}\includegraphics[width=0.5\linewidth]{figs/annotated_bessel.pdf}}\hfill
%
% \hspace{0.5em}
\subfigure[\footnotesize Shifted confluent hypergeometric (SCH) distribution]
{\label{fig:sch}\includegraphics[width=0.5\linewidth]{figs/annotated_sch.pdf}}%\vspace{-0.5em}
...
{\label{fig:matrix_results}\includegraphics[width=0.669\linewidth]{figs/matrix_results.pdf}}\hfill
%
\subfigure[Tensor empirical studies]
{\label{fig:tensor_results}\includegraphics[width=0.32\linewidth]{figs/tensor_results.pdf}}%\vspace{-0.5em}
\caption{\label{fig:empirical}\footnotesize The smoothing (top row) or forecasting (bottom row) performance of each model is quantified by its information gain over a non-dynamic baseline (BPTF ~\cite{schein2015bayesian}), where higher values are better.~\looseness=-1}%\vspace{-2em}
\end{figure*}
...
%
To quantify the performance of each model, we use $S$ posterior samples to approximate the ``information rate"~\cite{wallach2008structured} of the heldout set, measured in nats per heldout count---i.e., $R(\Delta) = -\frac{1}{|\Delta|}\sum_{(t,\osubs) \in \Delta} \log \Big[\frac{1}{S}\sum_{s=1}^S \textrm{Pois}\big(\ydt; \musdt \big) \Big]$---
% \begin{equation}
% R(\Delta) = -\frac{1}{|\Delta|}\sum_{(t,\osubs) \in \Delta} \log \Big[\frac{1}{S}\sum_{s=1}^S \textrm{Pois}\big(\ydt; \musdt \big) \Big]
% \end{equation}
where $\Delta$ is the set of multi-indices of the heldout counts and $\musdt$ is the expectation of heldout count $\ydt$ computed from the $s^{\textrm{th}}$ posterior sample. The information rate measures how many nats on average a model needs to compress heldout data; lower is better. The information rate is sometimes referred to as cross entropy It is equivalent to the log of perplexity~\citep{wallach2009evaluation} and the negative of log pointwise predictive density (LPPD)~\citep{gelman2014understanding}. In each study, we fit a simple non-dynamic baseline---Bayesian Poisson tensor factorization (BPTF) \cite{schein2015bayesian}---and use it to predict the heldout counts. This model assumes that the count tensors at different time steps are i.i.d.---i.e., $y_{\osubs}^{\mathsmaller{(t)}} \tsim \Pois{\mud}$. For each of the dynamic models, we report the ``information gain'' over BPTF, defined by BPTF's information rate minus theirs; higher is better.\looseness=-1
...
Each data set consists of a sequence of count tensors, each of which contains the $249 \ttimes 249 \ttimes 20$ event counts for that time step. For both data sets, we used months as time steps. For GDELT, we considered the date range 2003--2008 (i.e., $T \teq 72$); for ICEWS, we considered the date range 1995--2013 (i.e., $T \teq 228$). We also used multi-neuronal spike train recordings of macaque monkey motor cortexes collected and studied by~\citet{vyas2018neural} and~\citet{williams2018unsupervised}.  In this data set, a count $y^{\mathsmaller{(t)}}_{ij}$ is the number of times neuron $i$ spiked in trial $j$ during time step $t$. These counts form a sequence of $N \ttimes S$ matrices, where $N \teq 100$ is the number of neurons and $S \teq 1,716$ is the number of trials. We used 20-millisecond intervals as time steps, yielding $T \teq 162$. For each data set, we created three masks that each randomly select six heldout time steps in the range $[2, T\tm 2]$. We fit each model to each tensor and mask using two independent chains of 4,000 MCMC iterations, saving every $50^{\textrm{th}}$ posterior sample after the first 1,000 iterations to compute its information rate. We also fit BPTF using variational inference.~Following~\citet{schein2016poisson}, we set $K \teq 100$ for all models. We depict the results in \cref{fig:tensor_results}, where the error bars reflect variability across random masks.\looseness=-1

\begin{figure*}[t]
\centering
\subfigure[\footnotesize We visualize two components inferred by the sparse variant of the PRGDS (i.e., $\epstheta \teq 0$) from the ICEWS data set of international events. The blue component was also inferred by other models while the red component was not. The red component is specific to South Sudan, as revealed by visualizing the largest values of the sender and receiver factor vectors (bottom row, red). South Sudan was not a country until July 2011 when it gained independence from Sudan. The gamma states (top row, red) are thus sparse---i.e., $\thetakt \teq 0$ in 94\% of time steps (months) prior to July 2011 and in 83\% of the time steps overall. In contrast, the blue component represents Southeast Asian relations, which are active in all time steps. The sparse PRGDS can infer both temporally persistent latent structures (e.g., blue), as well as bursty latent structures that are highly localized in time (e.g., red).]
{
\label{fig:sudan}
% \includegraphics[width=\linewidth]{../../fig/components/icews/zero-ordering/new_plots/eps0-component-0.pdf}
\includegraphics[width=\linewidth]{figs/eps0-components72-0.pdf}
}
%
\hfill
\subfigure[\footnotesize We visualize components inferred by the PRGDS from the macaque motor cortex data set. The components inferred by the sparse PRGDS are bursty and highly localized in time (left), suggesting that neurons may be tuned to specific periods of the trial. The $K \ttimes T$ gamma latent states for this variant of the PRGDS are sparse (middle, white cells correspond to $\thetakt\teq 0$). The components (rows) are sorted by the time step in which the largest $\thetakt$ occurred, so the banded structure indicates that each component is only active for a short duration. In contrast, the components inferred by the non-sparse variant of the PRGDS are active in all time steps (right).]
{
\label{fig:macaque}
\includegraphics[width=0.49\linewidth]{figs/macaque_components_eps0.pdf}
\includegraphics[width=0.49\linewidth]{figs/macaque_heat.pdf}
}
%
%
%\caption{\label{fig:exploratory} International event data (\cref{fig:sudan}) and macaque motor cortex data (\cref{fig:macaque}).}
\caption{\label{fig:exploratory} The PRGDS infers sparse and temporally localized representations of two data sets.\looseness=-1}
\end{figure*}

\textbf{Discussion.} All of the dynamic models outperformed the non-dynamic baseline in all studies, as evidenced by all values of information gain being positive in \cref{fig:empirical}. In all sixteen studies, the sparse variant of the PRGDS (i.e., $\epstheta \teq 0$) obtained higher information gain over BPTF than the non-sparse variant (i.e., $\epstheta \teq 1$). In eight of the sixteen studies, the sparse PRGDS significantly outperformed the PGDS while the PGDS significantly outperformed it in only two. We conjecture that the better performance of the sparse variant can be explained by the form of the marginal expectation of $\boldsymbol{\theta}^{\mathsmaller{(t)}}$ (see
...
visualize example components in \cref{fig:exploratory}.
...
\section{Conclusion} We presented the Poisson-randomized gamma dynamical system (PRGDS), a tractable, expressive, and efficient model for sequentially observed count tensors. The PRGDS is based on a new modeling motif, an alternating chain of discrete Poisson and continuous gamma latent states that yields closed-form complete conditionals for all variables. We found that a sparse variant of the PRGDS, which allows the gamma latent states to take values of exactly zero, often obtains better smoothing and forecasting performance than other models and infers latent structures that are highly localized in time.\looseness=-1


{\small
  \textbf{Acknowledgments} \;
  We thank Saurabh Vyas, Alex Williams, and Krishna Shenoy for kindly sharing the macaque monkey motor cortex data set and their preprocessing code. SWL was supported by the Simons Collaboration on the Global Brain (SCGB 418011). MZ acknowledges the support of Award IIS-1812699 from the U.S. National Science Foundation. DMB is supported by ONR N00014-17-1-2131, ONR N00014-15-1-2209, NIH 1U01MH115727-01, NSF CCF-1740833, DARPA SD2 FA8750-18-C-0130, IBM, 2Sigma, Amazon, NVIDIA, and the Simons Foundation.}
