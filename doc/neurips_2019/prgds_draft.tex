...
\centering
% 
\subfigure[\footnotesize Latent activation structure inferred from macaque cortex data by the PrGDS. \emph{Right:} Comparison of the $K \ttimes T$ state matrix $\Theta$ inferred by the sparse versus non-sparse PrGDS variants. The components $k$ are sorted by which time step $t$ had the largest $\thetakt$---the visible banded structure suggests both infer components that activate within specific short durations. White cells correspond to zeros $\thetakt\teq 0$ which only the sparse PrGDS represent. \emph{Left:} Alternative visualization of ten rows of the matrix $\Theta$, each of which $\boldsymbol{\theta}_k$ is a $T$-length activation vector. The sparsity of each vector is provided in the legend. This visualization depicts latent dynamics which are bursty and temporally localized, conforming to standard neuroscience accounts of neuronal spiking dynamics. ~\looseness=-1]  
{
\label{fig:macaque}
