...
\begin{figure*}[t]
\centering
\subfigure[\footnotesize Matrix experiments (originally described by Schein et al.~\cite{schein2016poisson}).]
{\label{fig:matrix_results}\includegraphics[width=0.669\linewidth]{../../results/matrices/new_matrix_results.pdf}}\hfill
% 
\subfigure[Tensor experiments.]
{\label{fig:tensor_results}\includegraphics[width=0.32\linewidth]{../../results/tensors/new_tensor_results.pdf}}\vspace{-0.5em}
\caption{\label{fig:experiments}\footnotesize The smoothing (top row) and forecasting (bottom row) performance of each model is measured by log-perplexity---where \emph{lower is better}---divided by the log-perplexity of a non-dynamic baseline, BPTF \cite{schein2015bayesian}.~\looseness=-1}\vspace{-2em}
\end{figure*}
...
% 
% 
\subfigure[\footnotesize A component inferred by the sparse PrGDS from ICEWS data is specific to South Sudan, a country that did not exist until July 2011. Neither the non-sparse PrGDS variant nor the PGDS inferred such a component. We speculate that its strong inductive bias towards sparsity allows this variant to infer highly localized structure. \emph{Top:} The activation vector is sparse $\thetakt \teq 0$ at 94\% of the time steps (months) prior to July 2011. \emph{Bottom left and middle:} The factor vectors for the first two modes are dominated by South Sudan and Sudan.~\looseness=-1]
{
\label{fig:sudan}
...
% 
\caption{\label{fig:exploratory} Sparse representations of macaque cortex activity (\cref{fig:macaque}) and ICEWS events (\cref{fig:sudan}).}
\end{figure*}
